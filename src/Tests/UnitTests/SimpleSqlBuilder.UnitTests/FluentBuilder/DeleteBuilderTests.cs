...
public class DeleteBuilderTests
{
    ...
    [Theory]
    [AutoData]
    public void DeleteFrom_BuildsSqlWithWhereConditionalMethods_ReturnsFluentSqlBuilder(int id, int age, string type, int[] ages)
    {
        // Arrange
        var expectedSql = $"DELETE FROM Table{Environment.NewLine}" +
            "WHERE (Age < 100 OR Age = @p0 OR Age IN @pc1_) AND Type LIKE '%Type' AND (Age > 10 AND Type = @p2) OR Id NOT IN @pc3_";
    ...
    [Theory]
    [AutoData]
    public void DeleteFrom_BuildsSqlWithSimpleParameterInfoValuesAndReuseDisabled_ReturnsFluentSqlBuilder(int id, string type)
    {
        // Arrange
        var idParam = id.DefineParam(System.Data.DbType.Int32, 1, 1, 1, reuse: false);
        var typeParam = type.DefineParam(reuse: false);
        string expectedSql = $"DELETE FROM Table{Environment.NewLine}WHERE Id = @p0 AND Type = @p1 OR (Id = @p2 AND Type = @p3)";
