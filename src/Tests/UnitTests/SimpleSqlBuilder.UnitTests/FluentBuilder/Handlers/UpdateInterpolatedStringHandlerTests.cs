...
public class UpdateInterpolatedStringHandlerTests
{
    [Theory]
    [AutoData]
    public void Constructor_InitialiseHandler_HandlerInitialised(Mock<IFluentBuilder> fluentBuilderMock)
    {
        ...
        //Act
        var sut = new UpdateInterpolatedStringHandler(0, 0, fluentBuilderMock.Object);

        //Assert
        fluentSqlFormatterMock.Verify(x => x.StartClauseAction(ClauseAction.Update));
    }
    ...
    public void Close_ClauseActionEnded_ReturnsVoid(Mock<IFluentBuilder> fluentBuilderMock)
    {
        ...
        //Assert
        fluentSqlFormatterMock.Verify(x => x.EndClauseAction(ClauseAction.Update));
    }
}
#endif
