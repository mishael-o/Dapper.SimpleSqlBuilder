...
public class SelectDistinctInterpolatedStringHandlerTests
{
    ...
    public void Constructor_InitialisesHandler_ReturnsHandler()
    {
        ...
        // Act
        _ = new SelectDistinctInterpolatedStringHandler(0, 0, fluentBuilder);

        // Assert
        fluentFormatterMock.Received().StartClauseAction(ClauseAction.SelectDistinct);
    }
    ...
    public void AppendLiteral_AppendsLiteral_ReturnsVoid(string value)
    {
        // Arrange
        var fluentBuilder = Dapper.SimpleSqlBuilder.UnitTests.UnitTestSubstitutes.CreateFluentBuilder(out var fluentFormatterMock);
        var sut = new SelectDistinctInterpolatedStringHandler(0, 0, fluentBuilder);
