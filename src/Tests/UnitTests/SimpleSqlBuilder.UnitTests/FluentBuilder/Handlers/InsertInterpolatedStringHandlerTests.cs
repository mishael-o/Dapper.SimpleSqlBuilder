...
public class InsertInterpolatedStringHandlerTests
{
    ...
    public void Constructor_InitialisesHandler_ReturnsHandler()
    {
        ...
        // Act
        _ = new InsertInterpolatedStringHandler(0, 0, fluentBuilder);

        // Assert
        fluentFormatterMock.Received().StartClauseAction(ClauseAction.Insert);
    }
    ...
    public void AppendLiteral_AppendsLiteral_ReturnsVoid(string value)
    {
        // Arrange
        var fluentBuilder = Dapper.SimpleSqlBuilder.UnitTests.UnitTestSubstitutes.CreateFluentBuilder(out var fluentFormatterMock);
        var sut = new InsertInterpolatedStringHandler(0, 0, fluentBuilder);
