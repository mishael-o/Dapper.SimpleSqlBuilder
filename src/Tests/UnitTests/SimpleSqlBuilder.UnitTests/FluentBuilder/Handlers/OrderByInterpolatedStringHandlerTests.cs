...
public class OrderByInterpolatedStringHandlerTests
{
    [Theory]
    [AutoData]
    public void Constructor_InitialiseHandler_HandlerInitialised(Mock<IFluentBuilder> fluentBuilderMock)
    {
        ...
        fluentSqlFormatterMock.Verify(x => x.StartClauseAction(ClauseAction.OrderBy));
    }
    ...
    public void Close_ClauseActionEnded_ReturnsVoid(Mock<IFluentBuilder> fluentBuilderMock)
    {
        ...
        //Assert
        fluentSqlFormatterMock.Verify(x => x.EndClauseAction(ClauseAction.OrderBy));
    }
}
#endif
