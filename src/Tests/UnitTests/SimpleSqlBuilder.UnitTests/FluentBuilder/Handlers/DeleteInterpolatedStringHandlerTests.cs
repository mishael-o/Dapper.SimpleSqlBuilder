...
public class DeleteInterpolatedStringHandlerTests
{
    ...
    public void Constructor_InitialisesHandler_ReturnsHandler()
    {
        ...
        // Act
        _ = new DeleteInterpolatedStringHandler(0, 0, fluentBuilder);

        // Assert
        fluentFormatterMock.Received().StartClauseAction(ClauseAction.Delete);
    }
    ...
    public void AppendLiteral_AppendsLiteral_ReturnsVoid(string value)
    {
        // Arrange
        var fluentBuilder = Dapper.SimpleSqlBuilder.UnitTests.UnitTestSubstitutes.CreateFluentBuilder(out var fluentFormatterMock);
        var sut = new DeleteInterpolatedStringHandler(0, 0, fluentBuilder);
