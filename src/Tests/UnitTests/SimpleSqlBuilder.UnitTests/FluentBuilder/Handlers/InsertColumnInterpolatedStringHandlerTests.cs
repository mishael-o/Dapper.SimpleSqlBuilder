...
public class InsertColumnInterpolatedStringHandlerTests
{
    ...
    public void Constructor_InitialisesHandler_ReturnsHandler()
    {
        ...
        // Act
        _ = new InsertColumnInterpolatedStringHandler(0, 0, fluentBuilder);

        // Assert
        fluentFormatterMock.Received().StartClauseAction(ClauseAction.InsertColumn);
    }
    ...
    public void AppendLiteral_AppendsLiteral_ReturnsVoid(string value)
    {
        // Arrange
        var fluentBuilder = Dapper.SimpleSqlBuilder.UnitTests.UnitTestSubstitutes.CreateFluentBuilder(out var fluentFormatterMock);
        var sut = new InsertColumnInterpolatedStringHandler(0, 0, fluentBuilder);
